...
import { AlertCircle, Loader2, Edit, Edit2 } from "lucide-react";
import { updateTimetableAction } from "@/actions/timetable";

const DAYS_OF_WEEK = ['Sunday', 'Monday', 'Tuesday', 'Wednesday', 'Thursday', 'Friday', 'Saturday'];

export function ClientEditScheduleDialog({ schedule, programs, courses, sections, rooms, allFaculty, isList = false }) {
  const [open, setOpen] = useState(false);
  const [isPending, startTransition] = useTransition();
  const [error, setError] = useState(null);

  const initialProgram = schedule.program?.id || "";
  const initialCourse = schedule.subject?.id || "";
  const initialSection = schedule.section?.id || "";
  ...
  const handleSubmit = (e) => {
    ...
    startTransition(async () => {
      const result = await updateTimetableAction(schedule.id, formData);
      if (result?.error) {
      ...
      <DialogContent className="sm:max-w-[600px] max-h-[90vh] overflow-y-auto">
        <form onSubmit={handleSubmit}>
          <DialogHeader>
            ...
            <DialogDescription>
              Update class time or room. Conflicts will be checked automatically.
            </DialogDescription>
          </DialogHeader>
          ...
          <div className="grid gap-6 py-6">
            ...
            <div className="grid gap-4 md:grid-cols-2">
              ...
              <div className="grid gap-2">
                ...
                <Select name="faculty_id" defaultValue={schedule.faculty?.id} required disabled={isPending || !selectedCourse}>
                  <SelectTrigger>
            ...
            <div className="grid gap-4 md:grid-cols-2">
              <div className="grid gap-2">
                ...
                <Select name="room_id" defaultValue={schedule.room?.id} required disabled={isPending}>
                  <SelectTrigger>
              ...
              <div className="grid gap-2">
                ...
                <Select name="class_type" defaultValue={schedule.class_type} disabled={isPending}>
                  <SelectTrigger>
            ...
            <div className="grid gap-4 md:grid-cols-3">
              <div className="grid gap-2">
                ...
                <Select name="day_of_week" defaultValue={schedule.day_of_week.toString()} required disabled={isPending}>
                  <SelectTrigger>
              ...
              <div className="grid gap-2">
                ...
                <Input type="time" name="start_time" defaultValue={schedule.start_time.substring(0, 5)} required disabled={isPending} />
              </div>

              <div className="grid gap-2">
                <Label>End Time <span className="text-red-500">*</span></Label>
                <Input type="time" name="end_time" defaultValue={schedule.end_time.substring(0, 5)} required disabled={isPending} />
              </div>
            </div>
